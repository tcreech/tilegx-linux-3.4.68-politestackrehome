...
static int should_ptrace_signal(struct sighand_struct *sighand, int signr)
{
	struct k_sigaction *ka = &sighand->action[signr-1];
	if (!(ka->sa.sa_flags & SA_NOPTRACE))
		return 1;
	if (ka->sa.sa_handler == SIG_IGN || ka->sa.sa_handler == SIG_DFL)
		return 1;
	return 0;  /* don't trace handlers marked with SA_NOPTRACE */
}
...
relock:
	...
	for (;;) {
		...
		if (unlikely(current->ptrace) && signr != SIGKILL &&
		    should_ptrace_signal(sighand, signr)) {

...
/*
 * For each core, true when the core has issued rcu_user_enter()
 * more recently than rcu_user_exit(); this allows us to enter
 * and exit dynticks idle mode without generating warnings due to not
 * being in the idle task.
 */
DEFINE_PER_CPU(bool, in_dataplane);
#endif
...
#ifdef CONFIG_DATAPLANE
	if (!__this_cpu_read(in_dataplane))
#endif
...
{
	WARN_ON_ONCE(__this_cpu_read(in_dataplane));
	__this_cpu_write(in_dataplane, true);
	rcu_idle_enter();
}
#endif
...
void rcu_user_exit(void)
{
	rcu_idle_exit();
	WARN_ON_ONCE(!__this_cpu_read(in_dataplane));
	__this_cpu_write(in_dataplane, false);
}
#endif

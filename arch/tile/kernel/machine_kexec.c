...
#ifdef CONFIG_PCI
	/*
	 * We need to release all the TRIO resources that Linux allocated
	 * to avoid PCIe resource leakage.
	 */
	int i;

	for (i = 0; i < TILEGX_NUM_TRIO; i++)
		gxio_trio_destroy(&trio_contexts[i]);
#endif

       /*
	* ipi event registers should be reset after closing devices since
	* this ensures there's no unexpected ipi interrrupts which are
	* delivered to the 2nd kernel.
	*/
	__insn_mtspr(SPR_IPI_MASK_SET_K, -1UL);
	__insn_mtspr(SPR_IPI_EVENT_RESET_K, -1UL);
#endif
}

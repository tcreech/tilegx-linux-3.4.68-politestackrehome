...
void hv_message_intr(struct pt_regs *regs, int intnum)
{
	...
	while (1) {
		...
		if (dataplane_debug(smp_processor_id()))
			pr_err("cpu %d: dataplane recv msg len %d, src %d\n",
		...
		if (rmi.source == HV_MSG_TILE) {
			...
			/*
			 * These messages are not used by tilegx, so
			 * we are now folding them into the regular
			 * device interrupt (IPI) framework for statistics.
			 */

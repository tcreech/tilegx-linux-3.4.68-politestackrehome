		       ...
		       unsigned long reason)
{
	...
	case INT_MEM_ERROR:
		...
		break;
	case INT_ILL:
		if (notify_die(DIE_BREAK, "debug", regs, 0, INT_ILL,
			       SIGTRAP) == NOTIFY_STOP)
			return;
...
void kernel_double_fault(struct pt_regs *regs)
{
	struct KBacktraceIterator kbt;
	struct task_struct *tsk;

	tsk = validate_current_from_sp(regs->sp);
	pr_emerg("Kernel double fault: tid %d, pid %d (%s)"
		 " on cpu %d at cycle %lld\n",
		 tsk->pid, tsk->tgid, tsk->comm,
		 raw_smp_processor_id(), get_cycles());

	/*
	 * Note that we won't backtrace into userspace since "current"
	 * won't match "tsk" so the backtracer thinks we're another task.
	 * This turns out to be a requirement to avoid crashing in d_path()
	 * with a NULL "current" anyway.
	 */
	KBacktraceIterator_init(&kbt, tsk, regs);
	tile_show_stack(&kbt, 0);

	machine_halt();
}

void __kprobes do_double_fault(struct pt_regs *regs, int fault_num,
			       unsigned long reason)
{
	if (user_mode(regs))
		do_trap(regs, fault_num, reason);
	else
		kernel_double_fault(regs);
}

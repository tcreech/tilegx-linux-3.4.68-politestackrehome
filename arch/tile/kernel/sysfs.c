...
static ssize_t dataplane_show(struct device *dev,
			      struct device_attribute *attr,
			      char *page)
{
	int n = cpulist_scnprintf(page, PAGE_SIZE-2, &dataplane_map);
	page[n++] = '\n';
	page[n] = '\0';
	return n;
}
static DEVICE_ATTR(dataplane, 0444, dataplane_show, NULL);
#endif

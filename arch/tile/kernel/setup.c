...
#ifndef CONFIG_64BIT
unsigned long __initdata last_bootmem_pfn;
#endif
...
static void validate_dataplane(void)
{
	...
	if (hv_inquire_tiles(HV_INQ_TILES_SHARED,
		...
		printk("No support for inquiring hv shared tiles\n");
		return;
	}
	cpumask_and(&shared, &shared, &dataplane_map);
	if (cpumask_weight(&shared) != 0) {
		char buf[NR_CPUS * 5];
		cpulist_scnprintf(buf, sizeof(buf), &shared);
		pr_err("WARNING: removing hv shared tiles"
		       " from dataplane: %s\n", buf);
		cpumask_andnot(&dataplane_map, &dataplane_map, &shared);
	}
}
#endif

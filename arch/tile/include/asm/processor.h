...
struct thread_struct {
	...
	/* Support context-switching userspace_perf_counters. */
	unsigned long perf_count_ctl, aux_perf_count_ctl;
	unsigned long perf_count_0, perf_count_1;
	unsigned long aux_perf_count_0, aux_perf_count_1;
#endif
	...
	/* User ipi event mask */
	unsigned long ipi0_event_mask;
#endif
#ifdef CONFIG_DATAPLANE
	/* Flag bits for dataplane support (see <asm/dataplane.h>). */
	int dp_flags;
	/* When did we start waiting to quiesce? */
	unsigned long quiesce_start;
	/* Did we already warn about a long quiece period? */
	bool quiesce_warned;
#endif
};
...
extern void arch_exec_env(const char *envp);

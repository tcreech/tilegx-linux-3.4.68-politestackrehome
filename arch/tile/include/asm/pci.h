...
/*
 * We reserve the next 256MB below MEM_MAP_INTR_REGIONS_BASE to serve
 * as the PCI window, emulating the BAR2 space of an endpoint device.
 * This window is used by the chip-to-chip applications running on
 * the RC node to map the receive buffers to the PCI space.
 * PCIE_HOST_BAR2_SIZE depends on the number of Gx ports and the size 
 * of the receive buffer pool per port.
 */
#define PCIE_HOST_BAR2_SIZE		(16 * HPAGE_SIZE)
#define	PCIE_HOST_BAR2_START \
	(MEM_MAP_INTR_REGIONS_BASE - PCIE_HOST_BAR2_SIZE)

...
/*
 * The old tilepro architecture supports a 4KB page size (if the
 * Tilera hypervisor is recompiled that way), but it's not a
 * well-tested feature; tilegx doesn't support 4KB page sizes at all.
 * Using 4KB pages requires two pages per task stack, and this
 * unnecessarily complicates code that handles migration of the home
 * cache at process migration time.
 */
#if PAGE_SIZE == 4096
# error No support for 4KB page size
#endif
...
extern struct thread_info *alloc_thread_info_node(struct task_struct *task,
						  int node);
...
#define TIF_UPROBE		11	/* breakpointed or single-stepping */
...
#define _TIF_ALLWORK_MASK					\
	...
	_TIF_ASYNC_TLB|_TIF_NOTIFY_RESUME|_TIF_VIRT_EXIT|_TIF_UPROBE)

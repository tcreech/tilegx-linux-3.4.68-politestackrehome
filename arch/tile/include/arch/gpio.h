...
typedef union
{
  struct
  {
     ...
     * This vector contains the interrupts associated with the 64 pin
     * assertion interrupts.  Reading this register clears all of the
     * associated interrupts. Accesses to this register are masked by the
     * SVC_DOM PIN ENABLE field. If the PIN ENABLE bit is 0, the read bit
     * will be zero and the interrupt status will not be modified.
     */
    uint_reg_t int_vec0_rtc : 64;
#else   /* __BIG_ENDIAN__ */
    uint_reg_t int_vec0_rtc : 64;
#endif
  };
...
typedef union
{
  struct
  {
     ...
     * INT_VEC0_RTC.  This vector contains the interrupts associated with the
     * 64 pin assertion interrupts.  Writing a 1 clears the status bit.
     * Accesses to this register are masked by the SVC_DOM PIN ENABLE field.
     * If the PIN ENABLE bit is 0, the read bit will be zero and the written
     * bit will be ignored.
     */
    uint_reg_t int_vec0_w1tc : 64;
#else   /* __BIG_ENDIAN__ */
    uint_reg_t int_vec0_w1tc : 64;
#endif
  };
...
typedef union
{
  struct
  {
     ...
     * This vector contains the interrupts associated with the 64 pin
     * deassertion interrupts.  Reading this register clears all of the
     * associated interrupts. Accesses to this register are masked by the
     * SVC_DOM PIN ENABLE field. If the PIN ENABLE bit is 0, the read bit
     * will be zero and the interrupt status will not be modified.
     */
...
typedef union
{
  struct
  {
     ...
     * 64 pin deassertion interrupts.  Writing a 1 clears the status bit.
     * Accesses to this register are masked by the SVC_DOM PIN ENABLE field.
     * If the PIN ENABLE bit is 0, the read bit will be zero and the written
     * bit will be ignored.
     */
...
#ifndef __BIG_ENDIAN__
    /*
     * Pin n direction is controlled by PIN_DIR_O[n] and PIN_DIR_I[n],
     * encoded as:
     * 00b=Disabled
...
#ifndef __BIG_ENDIAN__
    /*
     * Pin n direction is controlled by PIN_DIR_O[n] and PIN_DIR_I[0],
     * encoded as:
     * 00b=Disabled
     * 01b=Input mode. Disable the output driver
     * 10b=Output mode.  Drive pin to logic level specified by data assertion
...
#ifndef __BIG_ENDIAN__
    /*
     * Each bit corresponds to a GPIO pin.  When 0, reads of the PIN_STATE
     * will return the input data for this pin.  When 1, reads of the
     * PIN_STATE register will return 0.
     */
    uint_reg_t msk : 64;
#else   /* __BIG_ENDIAN__ */
    uint_reg_t msk : 64;
...
typedef union
{
  struct
  {
     ...
     * PIN_STATE register for this bit will be processed normally. When the
     * bit is 1, writes to the PIN_STATE register for this bit will be
     * ignored and the drive level will not be modified.
     */
    uint_reg_t msk : 64;
#else   /* __BIG_ENDIAN__ */
    uint_reg_t msk : 64;
...
#ifndef __BIG_ENDIAN__
    /*
     * Writing a 1 to a bit will assert the output pin for a single time
     * slice.  Pins configured as outputs will return to the previous output
     * drive state and drive value after 1 time slice.
     */
    uint_reg_t pulse_set : 64;
#else   /* __BIG_ENDIAN__ */
    uint_reg_t pulse_set : 64;
#endif
  };
...
#ifndef __BIG_ENDIAN__
    /*
     * When read, returns the pin state after conditional inversion
     * controlled by PIN_INPUT_INV for all enabled pins and 0 for all
     * disabled pins. Pins are enabled for read if the bit is set in SVC_DOM
     * PIN_ENABLE field and clear in the PIN_INPUT_MSK. When written, the
     * value will be modified by PIN_OUTPUT_INV and will be driven on the
     * enabled output pins. Pins are enabled for output if the SVC_DOM
     * PIN_ENABLE field is set, PIN_DIR_O is set, and the PIN_OUTPUT_MSK is
     * clear.
     */
    uint_reg_t state : 64;
#else   /* __BIG_ENDIAN__ */

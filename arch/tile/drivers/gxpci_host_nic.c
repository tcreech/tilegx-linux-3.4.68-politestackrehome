...
{
	struct sk_buff *skb;
	skb = __alloc_skb(length + NET_SKB_PAD, GFP_ATOMIC, 0, numa_node_id());
	if (likely(skb)) {
			 ...
			 int *work_done, int work_to_do)
{
	...
	for (; last != new; last++) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
		    likely((new_skb =
			gxpci_netdev_alloc_skb_ip_align(netdev, pkt_len)) !=
			NULL)) {
#else
		...
		eth = eth_hdr(skb);
		skb->pkt_type = tile_rx_header->pkt_type;
...
static void gxpci_vlan_rx_add_vid(struct net_device *netdev, u16 vid)
{
	...
	gxpci_host_nic_reg_wr(nic, GXPCI_TILE_VLAN_FILTER_TABLE_ENTRY, value);

...
int gxio_gpio_set_dir(gxio_gpio_context_t * context, uint64_t disabled_pins,
		      ...
		      uint64_t output_od_pins)
{
	struct set_dir_param temp;
	struct set_dir_param *params = &temp;

	params->disabled_pins = disabled_pins;
	params->input_pins = input_pins;
	params->output_pins = output_pins;
	params->output_od_pins = output_od_pins;

	return hv_dev_pwrite(context->fd, 0, (HV_VirtAddr) params,
			     sizeof(*params), GXIO_GPIO_OP_SET_DIR);
}

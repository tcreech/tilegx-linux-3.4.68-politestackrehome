			    ...
			    int inter_y, int inter_ipi, int inter_event)
{
	...
	return hv_dev_pwrite(context->fd, 0, (HV_VirtAddr) params,
			     sizeof(*params), GXIO_I2CS_OP_CFG_INTERRUPT);
}

EXPORT_SYMBOL(gxio_i2cs_cfg_interrupt);
...
int gxio_i2cs_get_mmio_base(gxio_i2cs_context_t * context, HV_PTE *base)
{
	...
	__result =
	    hv_dev_pread(context->fd, 0, (HV_VirtAddr) params, sizeof(*params),
			 GXIO_I2CS_OP_GET_MMIO_BASE);
	*base = params->base;

	return __result;
}

EXPORT_SYMBOL(gxio_i2cs_get_mmio_base);
				...
				unsigned long offset, unsigned long size)
{
	...
	return hv_dev_pwrite(context->fd, 0, (HV_VirtAddr) params,
			     sizeof(*params), GXIO_I2CS_OP_CHECK_MMIO_OFFSET);
}

EXPORT_SYMBOL(gxio_i2cs_check_mmio_offset);

...
void show_mem(unsigned int filter)
{
	...
	pr_err(" sequestered:%lu/%lu\n",
	       homecache_count_sequestered_pages(0, 1),
	       homecache_count_sequestered_pages(1, 1));
#endif
...
void shatter_huge_page(unsigned long addr)
{
	...
	static DEFINE_SPINLOCK(shatter_lock);
	...
	/*
	 * Take the lock and examine the pmd to see if we need to
	 * shatter.  If we didn't always take the lock, we would
	 * be exposed to the possibility that a shatter is currently
	 * underway on another core, and that the new page-table entry
	 * is present but the TLB still holds the old huge page.  This
	 * could cause cache incoherence if the caller then adjusts
	 * the small page homing, does a small-page TLB flush, and
	 * touches the page all before the remote flush from the
	 * shattering core reaches the re-homing core.
	 *
	 * Note that even though we are manipulating the init_mm
	 * page table, we can't use the page_table_lock since the
	 * shattering code can be called from an irq-unsafe context.
	 * We don't anticipate any other user ever shattering a kernel
	 * huge page, so we believe it is safe here to simply lock
	 * internally within this one function.
	 */
	spin_lock_irqsave(&shatter_lock, flags);

	BUG_ON(!pmd_present(*pmd));
	if (!pmd_huge_page(*pmd)) {
		spin_unlock_irqrestore(&shatter_lock, flags);
		return;
	}
...
pte_t *virt_to_shattered_kpte(unsigned long vaddr)
{
...
void set_pte(pte_t *ptep, pte_t pte)
{
	...
	/* No standard path is allowed to set a migrating PTE. */
	BUG_ON(pte_migrating(pte));

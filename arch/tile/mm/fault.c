					  ...
					  struct pt_regs *regs)
{
	...
	current->thread.trap_nr = si_code;
...
static void wait_for_migration(unsigned long address, pte_t *pte)
{
	if (pte_migrating(*pte)) {
		 ...
		 */
		long retries = 0;
		long bound = get_clock_rate();
		while (pte_migrating(*pte)) {
			barrier();
			if (++retries > bound)
				homecache_debug_stuck_page(address, pte);
		}
	}
}
				...
				int is_kernel_mode, int write)
{
	...
	if (pte_migrating(pteval)) {
		...
		if (current->thread.homecache_is_migrating &&
			...
			return 0;
#endif
		wait_for_migration(address, pte);
		return 1;
	}
		   ...
		   unsigned long address, unsigned long write)
{
	...
	if (write & ~1) {
		...
		if (pc >= (unsigned long) __start_unalign_asm_code &&
			...
			/*
			 * At this point all bets are off; we may even be
			 * running on the user stack instead of the kernel
			 * stack, but somehow we seem to have a working
			 * stack.  We are basically in a double-fault
			 * condition, so treat it that way.
			 */
			pr_emerg("cpu %d: unexpected hypervisor ICS page fault "
				 " at address %#lx\n",
				 smp_processor_id(), address);
			pr_emerg("Previous fault context: PC %#lx, EX1 %#lx\n",
				 regs->pc, regs->ex1);
			regs->pc = pc;
			regs->ex1 = PL_ICS_EX1(KERNEL_PL, 1);
			tile_show_regs(regs);

...
static char *__initdata default_hstate_max_huge_pages_node;
static void __init hugetlb_nrpages_pernode_setup(struct hstate *h, char *s)
{
	int nid;
	unsigned long npages = 0;

	if (h->order >= MAX_ORDER) {
		/* FIXME: This is not too hard to get right, just more code. */
		printk("hugepages=%s: pages too big for per-node alloc\n", s);
		return;
	}
	...
	for (nid = 0; *s; ++nid) {
		char *end;
		unsigned long i;
		unsigned long pages = simple_strtoul(s, &end, 10);
		if (end == s)
			break;
		if (nid >= MAX_NUMNODES) {
			printk("hugepages: ignoring extra text \"%s\"\n", s);
			break;
		}
		s = (*end == ',') ? end+1 : end;

		for (i = 0; i < pages; ++i) {
			if (!alloc_fresh_huge_page_node(h, nid))
				break;
			npages++;
		}
	}

	h->max_huge_pages = npages;
}
...
static int __init hugetlb_init(void)
{
	...
	if (default_hstate_max_huge_pages_node)
		hugetlb_nrpages_pernode_setup(
			&default_hstate, default_hstate_max_huge_pages_node
			);
...
static int __init hugetlb_nrpages_setup(char *s)
{
	...
	if (strchr(s, ',') != NULL) {
		if (!max_hstate)
			default_hstate_max_huge_pages_node = s;
		else
			hugetlb_nrpages_pernode_setup(parsed_hstate, s);
		last_mhp = mhp;
		return 1;
	}
...
out_mutex:
	...
	/* Any OOM fault brought up to this level just means we ran out
	 * of hugepages, and for userspace to be able to recover, we need
	 * to deliver a SIGBUS.
	 * FIXME: not clear that this is still true in Linux 3.0.
	 */
	if (ret == VM_FAULT_OOM)
		ret = VM_FAULT_SIGBUS;

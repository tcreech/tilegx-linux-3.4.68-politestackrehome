...
static int pkmap_flushes, pkmap_waits;
...
static ctl_table pkmap_table[] = {
	{
		.procname	= "pkmap_flushes",
		.data		= &pkmap_flushes,
		.maxlen		= sizeof(int),
		.mode		= 0444,
...
static int __init pkmap_proc_init(void)
{
	register_sysctl_table(pkmap_root);
	return 0;
}
subsys_initcall(pkmap_proc_init);

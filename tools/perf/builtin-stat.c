...
static struct perf_event_attr default_attrs[] = {
   ...
   * FIXME: why do no other architectures customize this list?
   */
...
#ifdef __tilegx__
		/*
		 * Tile does not have a branch instruction counter, but
		 * instead branch-mispredicted and branch-predicted counters,
		 * so we need to do a suitable adjustment here.
		 */
		ratio = avg / (total + avg) * 100.0;
#else
		ratio = avg / total * 100.0;
#endif

...
#if defined(USE_SIM_PRINTF)
/*
 * Tag this with TILERA_INTERNAL since it is not suitable for
...
static struct mpipe_data {
	...
	*tile_net_devs_for_channel[TILE_NET_CHANNELS];
	...
	/* cookie for the link status interrupt. */
	int link_irq_cookie;

	/* Is the mpipe instance initialized in driver? */
	bool is_initialized;
...
static bool network_cpus_init(void)
{
	...
	cpumask_andnot(&network_cpus_map, &network_cpus_map, &dataplane_map);
...
#if defined(TILE_NET_DUMP_PACKETS)
/* Dump a packet. */
...
#if defined(TILE_NET_PARANOIA)
        /* Removing this check increased packets per second by 5% when
	 * tested with iperf. It also allows the function to be inlined.
	 */
	pte_t *ptep = virt_to_pte(current->mm, (unsigned long)skb);
	bool valid = (ptep != NULL && pte_present(*ptep) && pte_read(*ptep));

	/* Paranoia. */
	if (!valid || skb->data != va) {
				 ...
				 gxio_mpipe_idesc_t *idesc, unsigned long len)
{
	...
	if ((dev->features & NETIF_F_RXCSUM) && idesc->cs &&
			idesc->csum_seed_val == 0xFFFF)
...
#if 0
		if (net_ratelimit())
			pr_info("Dropping bad packet (i0=0x%lx).\n",
				(long)idesc->words[0]);
#endif
		goto drop;
	}
...
void tile_net_link_check(struct mpipe_data *md)
{
	struct tile_net_priv *priv;
	struct net_device *dev;
	unsigned long current_state;
	int channel, link_speed;

	for (channel = 0; channel < TILE_NET_CHANNELS; channel++) {
		dev = md->tile_net_devs_for_channel[channel];
		if (dev == NULL)
			continue;

		priv = netdev_priv(dev);
		current_state = gxio_mpipe_link_get_attr(&priv->link,
			GXIO_MPIPE_LINK_CURRENT_STATE);
		link_speed = current_state & GXIO_MPIPE_LINK_SPEED_MASK;
		if (priv->link_up != (!!link_speed)) {
			priv->link_up = !!link_speed;
...
/* Handle a link interrupt from an instance. */
static irqreturn_t tile_net_handle_link_irq(int irq, void *id)
{
	struct mpipe_data *md = &mpipe_data[(uint64_t)id];

	gxio_mpipe_link_arm_pollfd(&md->context, md->link_irq_cookie);
	tile_net_link_check(md);
...
static enum hrtimer_restart tile_net_handle_egress_timer(struct hrtimer *t)
{
	...
	for (instance = 0; instance < NR_MPIPE_MAX &&
		     mpipe_data[instance].is_initialized; instance++) {
...
static void register_ptp_clock(struct net_device *dev, struct mpipe_data *md)
{
	...
	md->ptp_clock = ptp_clock_register(&md->caps);
...
/* Update the hv link mask for link irq. */
static int tile_net_cfg_link_irq(struct net_device *dev)
{
	int instance = mpipe_instance(dev);
	struct mpipe_data *md = &mpipe_data[instance];
	int rc, cpu;

	cpu = cpumask_first(&network_cpus_map);
	rc = gxio_mpipe_link_cfg_pollfd(&md->context,
					cpu_x(cpu), cpu_y(cpu),
					KERNEL_PL, md->link_irq, -1);
	if (rc < 0) {
		netdev_err(dev, "gxio_mpipe_link_cfg_pollfd failed: "
			   "mpipe[%d] %d\n", instance, rc);
		return rc;
	}
	md->link_irq_cookie = rc;

	rc = gxio_mpipe_link_arm_pollfd(&md->context, md->link_irq_cookie);
	if (rc < 0) {
		netdev_err(dev, "gxio_mpipe_link_arm_pollfd failed: "
			   "mpipe[%d] %d\n", instance, rc);
		gxio_mpipe_link_close_pollfd(&md->context,
					     md->link_irq_cookie);
		md->link_irq_cookie = -1;
		return rc;
	}

	/*
	 * It is possible there's link interrupt missing before the
	 * interrupt is re-armed, so check the link status additionally.
	 */
	tile_net_link_check(md);
...
static int tile_net_update(struct net_device *dev)
{
	...
	if (!saw_channel) {
		...
		/* FIXME: We should probably do this in driver's shutdown()
		 * interface, which undoes many per-mpipe setup. e.g. see
		 * tile_net_init_mpipe_fail().
		 */
		if (md->link_irq_cookie >= 0) {
			gxio_mpipe_link_close_pollfd(&md->context,
						     md->link_irq_cookie);
			md->link_irq_cookie = -1;
		}
	...
	if (irq < 0) {
		...
		if (irq < 0) {
			netdev_err(dev,
				   "create_irq failed: mpipe[%d] %d\n",
				   instance, irq);
			destroy_irq(md->ingress_irq);
			return irq;
		}
		tile_irq_activate(irq, TILE_IRQ_SW_CLEAR);

		rc = request_irq(irq, tile_net_handle_link_irq,
				 0, "tile_net", (void *)((uint64_t)instance));
		if (rc != 0) {
			netdev_err(dev, "request_irq failed: mpipe[%d] %d\n",
				   instance, rc);
...
static int tile_net_open(struct net_device *dev)
{
	...
	/* Initialize the link state as "down". */
	netif_carrier_off(dev);
	priv->link_up = false;
	...
	priv->instance = rc;
	instance = rc;
	if (!mpipe_data[rc].is_initialized) {
...
static int tile_net_get_settings(struct net_device *netdev,
				 struct ethtool_cmd *ecmd)
{
	struct tile_net_priv *priv = netdev_priv(netdev);
	unsigned long current_state;
	int link_speed = 0;
	...
	switch (link_speed) {
	case GXIO_MPIPE_LINK_10M:
		link_speed = SPEED_10;
		break;
	case GXIO_MPIPE_LINK_100M:
		link_speed = SPEED_100;
		break;
	case GXIO_MPIPE_LINK_1G:
		link_speed = SPEED_1000;
		break;
	case GXIO_MPIPE_LINK_10G:
...
static struct ethtool_ops tile_net_ethtool_ops = {
	.get_settings	= tile_net_get_settings,
...
static void tile_net_setup(struct net_device *dev)
{
	...
	SET_ETHTOOL_OPS(dev, &tile_net_ethtool_ops);
...
static int __init tile_net_init_module(void)
{
	...
	if (!network_cpus_init())
		cpus_andnot(network_cpus_map, *cpu_online_mask, dataplane_map);
#else
	if (!network_cpus_init())

...
static int __devinit pci_bus_get_depth(struct pci_bus *bus)
{
	int depth = 0;
	struct pci_dev *dev;
...
static int __devinit pci_get_max_depth(void)
{
	int depth = 0;
	struct pci_bus *bus;
...
static enum enable_type pci_realloc_enable __devinitdata = undefined;
